...
__global__ void sgemm_tile2d_transpose_kernel(const float *A, const float *B, float *C, int M, int N, int K, float alpha, float beta) {
...
void sgemm_tile2d_transpose(const float *A, const float *B, float *C, int M, int N, int K, float alpha, float beta) {
    ...
    sgemm_tile2d_transpose_kernel << < dimGrid, dimBlock >> > (A, B, C, M, N, K, alpha, beta);
}
